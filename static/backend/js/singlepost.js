                    ...
                    <Navigation />
                        <div className="main-wrapper">
                            ...
                            <main className="main  main--single-post">
                                <PostEditor postData={this.state.postData} />
                            </main>
                            <aside>
                                {/*<Actions />*/}
...
class PostEditor extends React.Component {
    ...
    changeOrder(e) {
        ...
        var postDataCopy = this.state.postData;

        var dir = e.target.value.split(',')[0], contentId = e.target.value.split(',')[1];

        var index = content.findIndex(contentModule => contentModule['_id']['$oid'] === contentId);

        let itemToMove = content.splice(index, 1)[0];

        var newIndex = (new Number(index + dir)).clamp(0, this.state.postData.content.length);
        content.splice(newIndex, 0, itemToMove);

        postDataCopy.content = content;

        client.put('/api/v1/posts/' + this.postId, {
            content: content.map(contentModule => contentModule['_id']['$oid'])
        })
        .then(res => {
            console.log(res);
            this.setState({
    ...
    render() {
        ...
        return (
            <article className="post-editor">
                ...
                />
                <div id={"sortable-" + this.postId}>{contentModules}</div>
